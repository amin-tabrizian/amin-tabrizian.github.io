---
layout: archive
title: "Talks"
permalink: /talks/
author_profile: true
---

## Invited Talks

**End-to-End LLM Flight Planning with RAG-based Memory and Multi-modal Coach Agent**  
Invited Technical Talk, Honeywell Advanced Technology, December 2025

**End-to-End LLM Flight Planning with RAG-based Memory and Multi-modal Coach Agent**  
**A. Tabrizian**, A. Aziz, A. Ullah, P. Razzaghi, P. Wei  
INFORMS Annual Meeting, Atlanta, GA, 2025

**Chain-of-Thought Flight Planner: End-to-End LLM Routing Under Wind Hazards**  
**A. Tabrizian**, M. Ghazanfari, P. Wei  
AIAA Aviation Forum, Las Vegas, NV, 2025  
**(Best Student Paper Award Winner)**

**Using Large Language Models to Automate Flight Planning Under Wind Hazards**  
**A. Tabrizian**, P. Gupta, A. Taye, J. Jones, E. Thompson, S. Chen, T. Bonin, D. Eberle, P. Wei  
AAAI 2025 Spring Symposium Series, San Francisco, CA, 2025

**RAPID: Reinforcement Learning-Aided Femtocell Placement for Indoor Drone Localization**  
A. Famili, **A. Tabrizian**, T. Atalay, A. Stavrou, P. Wei  
IEEE International Conference on Computer Communications and Networks (ICCCN), Big Island, HI, 2024

**Using Large Language Models to Automate Flight Planning under Wind Hazards**  
**A. Tabrizian** et al.  
INFORMS Annual Meeting, Seattle, WA, 2024
